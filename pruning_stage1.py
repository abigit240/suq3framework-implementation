...
class StructuredPruning:
    """Stage 1: Structured MxN block pruning"""
    
    def __init__(self, baseline_model, X_train, batch_size=config.BATCH_SIZE):
        """
    ...
    def _create_pruning_schedule(self):
        ...
        schedule = tfmot.sparsity.keras.PolynomialDecay(
            initial_sparsity=config.STRUCTURED_PRUNING_CONFIG['initial_sparsity'],
            final_sparsity=config.STRUCTURED_PRUNING_CONFIG['final_sparsity'],
            begin_step=config.STRUCTURED_PRUNING_CONFIG['begin_step'],
            end_step=end_step
        )
        return schedule
    
    def build_pruned_model(self):
        """Build model with structured pruning wrappers"""
        schedule = self._create_pruning_schedule()
        
        pruning_params = {
            'pruning_schedule': schedule,
            'block_size': config.STRUCTURED_PRUNING_CONFIG['block_size'],
            'block_pooling_type': config.STRUCTURED_PRUNING_CONFIG['block_pooling_type']
        }
        ...
        # Transfer weights from baseline
        self.model.layers[1].set_weights(self.baseline_model.model.layers[1].get_weights())  # Reshape
        self.model.layers[2].set_weights(self.baseline_model.model.layers[2].get_weights())  # GRU
        self.model.layers[3].layer.set_weights(self.baseline_model.model.layers[3].get_weights())  # Dense 1
        self.model.layers[4].layer.set_weights(self.baseline_model.model.layers[4].get_weights())  # Dense 2
        ...
        print("Structured pruning model built")
        return self.model
    ...
    def save(self, path):
        ...
        print(f"Structured pruned model saved to {path}")
